// FillStyleWoodDlg.h : Declaration of the CFillStyleWoodDlg
...
class CFillStyleWoodDlg : 
	public CComObjectRootEx<CComMultiThreadModel>,
	public Win32LangEx::CLangDialogImpl<CFillStyleWoodDlg>,
	public CObserverImpl<CFillStyleWoodDlg, ISharedStateObserver, TSharedStateChange>,
	public CDialogResize<CFillStyleWoodDlg>,
	public CContextHelpDlg<CFillStyleWoodDlg>,
	public CChildWindowImpl<CFillStyleWoodDlg, IRasterImageEditToolWindow>
{
public:
	CFillStyleWoodDlg() : m_bEnableUpdates(false)
	{
	}
	~CFillStyleWoodDlg()
	{
	...
	enum { IDD = IDD_FILLSTYLE_WOOD };

	BEGIN_MSG_MAP(CFillStyleWoodDlg)
		CHAIN_MSG_MAP(CContextHelpDlg<CFillStyleWoodDlg>)
		...
		COMMAND_HANDLER(IDC_ETW_RINGS, EN_CHANGE, OnChangeEdit)
		COMMAND_HANDLER(IDC_ETW_TURBULENCE, EN_CHANGE, OnChangeEdit)
		COMMAND_HANDLER(IDC_ETW_FIBRES, EN_CHANGE, OnChangeEdit)
		COMMAND_HANDLER(IDC_ETW_GAIN, EN_CHANGE, OnChangeEdit)
		...
		CHAIN_MSG_MAP(CDialogResize<CFillStyleWoodDlg>)
		REFLECT_NOTIFICATIONS();
	END_MSG_MAP()

	BEGIN_DLGRESIZE_MAP(CFillStyleWoodDlg)
		...
		DLGRESIZE_CONTROL(IDC_ETW_RINGS, DLSZ_SIZE_X)
		DLGRESIZE_CONTROL(IDC_ETW_TURBULENCE, DLSZ_SIZE_X)
		DLGRESIZE_CONTROL(IDC_ETW_FIBRES, DLSZ_SIZE_X)
		DLGRESIZE_CONTROL(IDC_ETW_GAIN, DLSZ_SIZE_X)
	END_DLGRESIZE_MAP()

	BEGIN_CTXHELP_MAP(CFillStyleWoodDlg)
	...
	BEGIN_COM_MAP(CFillStyleWoodDlg)
	...
	HWND Create(LPCOLESTR a_pszToolID, HWND a_hParent, LCID a_tLocaleID, ISharedStateManager* a_pSharedState, BSTR a_bstrSyncGroup)
	{
		...
		return Win32LangEx::CLangDialogImpl<CFillStyleWoodDlg>::Create(a_hParent);
	}
...
public:
	STDMETHOD(PreTranslateMessage)(MSG const* a_pMsg, BOOL a_bBeforeAccel)
	{
		if (a_bBeforeAccel && m_hWnd && a_pMsg->hwnd && (
			...
			a_pMsg->hwnd == m_wndRings || a_pMsg->hwnd == m_wndTurbulence ||
			a_pMsg->hwnd == m_wndFibres || a_pMsg->hwnd == m_wndGain))
		{
	...
	LRESULT OnInitDialog(UINT a_uMsg, WPARAM a_wParam, LPARAM a_lParam, BOOL& a_bHandled)
	{
		...
		TColor const tColor1 = CFillStyleDataWood::DefaultColor1();
		...
		TColor const tColor2 = CFillStyleDataWood::DefaultColor2();
		...
		m_wndRings = GetDlgItem(IDC_ETW_RINGS);
		m_wndTurbulence = GetDlgItem(IDC_ETW_TURBULENCE);
		m_wndFibres = GetDlgItem(IDC_ETW_FIBRES);
		m_wndGain = GetDlgItem(IDC_ETW_GAIN);
		...
		m_aMap[3] = std::make_pair(&m_wndRings, &m_cData.rings);
		m_aMap[4] = std::make_pair(&m_wndTurbulence, &m_cData.turbulence);
		m_aMap[5] = std::make_pair(&m_wndFibres, &m_cData.fibres);
		m_aMap[6] = std::make_pair(&m_wndGain, &m_cData.gain);

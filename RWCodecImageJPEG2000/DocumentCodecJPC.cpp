// DocumentCodecJPC.cpp : Implementation of CDocumentCodecJPC

#include "stdafx.h"
#include "DocumentCodecJPC.h"
...
STDMETHODIMP CDocumentCodecJPC::DefaultConfig(IConfig** a_ppDefCfg)
{
	try
	{
		*a_ppDefCfg = NULL;
...
STDMETHODIMP CDocumentCodecJPC::CanSerialize(IDocument* a_pDoc, BSTR* a_pbstrAspects)
{
	try
	{
...
STDMETHODIMP CDocumentCodecJPC::Serialize(IDocument* a_pDoc, IConfig* a_pCfg, IReturnedData* a_pDst, IStorageFilter* UNREF(a_pLocation), ITaskControl* UNREF(a_pControl))
{
	try
	{
		...
		{
			BYTE const* p = cBuffer.aData + cBuffer.nStride*y + 2;
		...
		for (int y = 0; y < data->numrows_; ++y)
		{
			BYTE const* p = cBuffer.aData + cBuffer.nStride*y + 1;
			for (int x = 0; x < data->numcols_; ++x)
			{
				jas_matrix_set(data, y, x, *p);
				p += 4;
		...
		for (int y = 0; y < data->numrows_; ++y)
		{
			BYTE const* p = cBuffer.aData + cBuffer.nStride*y;
			for (int x = 0; x < data->numcols_; ++x)
			{
				jas_matrix_set(data, y, x, *p);
				p += 4;
		...
		if (nComps == 4)
		{
			for (int y = 0; y < data->numrows_; ++y)
			{
				BYTE const* p = cBuffer.aData + cBuffer.nStride*y + 3;
				for (int x = 0; x < data->numcols_; ++x)
				{
					jas_matrix_set(data, y, x, *p);
					p += 4;
...
HRESULT CDocumentCodecJPC::Parse(ULONG a_nLen, BYTE const* a_pData, IStorageFilter* UNREF(a_pLocation), IDocumentFactoryRasterImage* a_pBuilder, BSTR a_bstrPrefix, IDocumentBase* a_pBase, GUID* a_pEncoderID, IConfig** a_ppEncoderCfg, ITaskControl* UNREF(a_pControl))
{
	SJasPerImageSource sStream(a_nLen, a_pData);
	if (jpc_validate(&sStream))
		return E_RW_UNKNOWNINPUTFORMAT;

	SAutoDeleteJasImage image(jpc_decode(&sStream, ""));
	if (image.p == NULL)
		return E_FAIL;
	...
	{
		a_pBase->EncoderSet(__uuidof(DocumentCodecJPC), NULL);
	}

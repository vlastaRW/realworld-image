// EditToolLineDlg.h : Declaration of the CEditToolLineDlg
...
class CEditToolLineDlg : 
	public CEditToolDlgBase<CEditToolLineDlg, CEditToolDataLine, Win32LangEx::CLangIndirectDialogImpl<CEditToolLineDlg> >,
	public CDialogResize<CEditToolLineDlg>,
	public CContextHelpDlg<CEditToolLineDlg>
{
public:
	CEditToolLineDlg()
	{
	...
	BEGIN_CONTROLS_MAP()
		CONTROL_CONTROL(_T(""), IDD_STROKE_MODES, TOOLBARCLASSNAME, TBSTYLE_TRANSPARENT | TBSTYLE_LIST | TBSTYLE_FLAT | TBSTYLE_TOOLTIPS | CCS_NOMOVEY | CCS_NORESIZE | CCS_NOPARENTALIGN | CCS_NODIVIDER | WS_TABSTOP | WS_VISIBLE, 5, 5, 90, 14, 0)
	...
	BEGIN_MSG_MAP(CEditToolLineDlg)
		CHAIN_MSG_MAP(CContextHelpDlg<CEditToolLineDlg>)
		...
		CHAIN_MSG_MAP(CDialogResize<CEditToolLineDlg>)
	END_MSG_MAP()

	BEGIN_DLGRESIZE_MAP(CEditToolLineDlg)
	...
	BEGIN_CTXHELP_MAP(CEditToolLineDlg)
	...
	BEGIN_COM_MAP(CEditToolLineDlg)
	...
	void OwnerNotify(TCookie, TSharedStateChange a_tParams)
	{
		...
		{
			CComQIPtr<CEditToolDataLine::ISharedStateToolData> pData(a_tParams.pState);
			if (pData)
			{
	...
	LRESULT OnInitDialog(UINT a_uMsg, WPARAM a_wParam, LPARAM a_lParam, BOOL& a_bHandled)
	{
		...
		if (m_pSharedState != NULL)
		{
			...
			m_pSharedState->StateGet(m_bstrSyncToolData, __uuidof(CEditToolDataLine::ISharedStateToolData), reinterpret_cast<void**>(&pState));
			if (pState != NULL)
			{
		...
		m_wndToolBar = GetDlgItem(IDD_STROKE_MODES);
	...
	void GUIToData()
	{
		...
		m_cData.eJoinMode = m_wndToolBar.IsButtonChecked(IDC_STROKE_MITERJOIN) ? agg::miter_join :
			(m_wndToolBar.IsButtonChecked(IDC_STROKE_ROUNDJOIN) ? agg::round_join : agg::bevel_join);
		TCHAR szVal[32] = _T("");
		m_wndWidth.GetWindowText(szVal, itemsof(szVal));
		szVal[itemsof(szVal)-1] = _T('\0');
	...
	void DataToGUI()
	{
		...
		m_wndToolBar.CheckButton(IDC_STROKE_MITERJOIN, m_cData.eJoinMode == agg::miter_join);
		m_wndToolBar.CheckButton(IDC_STROKE_ROUNDJOIN, m_cData.eJoinMode == agg::round_join);
		m_wndToolBar.CheckButton(IDC_STROKE_BEVELJOIN, m_cData.eJoinMode == agg::bevel_join);

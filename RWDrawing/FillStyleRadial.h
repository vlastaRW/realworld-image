...
public:
	...
	BEGIN_COM_MAP(CFillStyleRadial)
...
public:
	...
	STDMETHOD(GetBrushTile)(ULONG a_nX, ULONG a_nY, ULONG a_nSizeX, ULONG a_nSizeY, float a_fGamma, ULONG a_nStride, TRasterImagePixel* a_pBuffer)
	{
		...
		{
			float const s = sinf(m_fAngle);
			float const c = cosf(m_fAngle);
			float const f11 = c;
			float const f12 = s;
			float const f21 = -s/m_fAspectRatio;
			float const f22 = c/m_fAspectRatio;
			for (ULONG y = 0; y < a_nSizeY; ++y)
			{
				...
				for (ULONG x = 0; x < a_nSizeX; ++x)
				{
					...
					float const fDX2 = f11*fDX + f12*fDY;
					float const fDY2 = f21*fDX + f22*fDY;
					float fD = sqrtf(fDX2*fDX2 + fDY2*fDY2)/m_fRadius;
					if (fD > 1.0f) fD = 1.0f;
		...
		else
		{
			for (ULONG y = 0; y < a_nSizeY; ++y)
			{
				...
				{
					float const fDX = a_nX+x-m_tCenter.fX+0.5f;
					float fD = sqrtf(fDX*fDX + fDY*fDY)/m_fRadius;
					if (fD > 1.0f) fD = 1.0f;
					*a_pBuffer = cMap[BYTE(fD*255.0f+0.5f)];
					++a_pBuffer;
				}
	...
	STDMETHOD(GetControlPoint)(ULONG a_nIndex, TPixelCoords* a_pPos, ULONG* a_pClass)
	{
		try
		{
			...
			*a_pPos = m_tCenter;
			...
			{
				a_pPos->fX -= m_fRadius*sinf(m_fAngle)*m_fAspectRatio;
				a_pPos->fY += m_fRadius*cosf(m_fAngle)*m_fAspectRatio;
				*a_pClass = 1;
			}
			else
			{
	...
	STDMETHOD(SetControlPoint)(ULONG a_nIndex, TPixelCoords const* a_pPos, boolean a_bReleased, float a_fPointSize)
	{
		try
		{
			...
			else
			{
				...
				if (dist > 0.0f)
				{
					m_fAspectRatio = sqrtf(dist)/m_fRadius;
				}
			}
			if (m_pOwner)
			{
...
public:
	STDMETHOD(FromText)(BSTR a_bstrParams)
	{
		try
		{
			...
			bool bParsed = parse(a_bstrParams, a_bstrParams+SysStringLen(a_bstrParams),
					...
					(!(ch_p(L',')>>real_p[assign_a(fAspectRatio)]>>ch_p(L',')>>real_p[assign_a(fAngle)]))
					).full;
			if (!bParsed)
				return E_INVALIDARG;
	...
	STDMETHOD(ToText)(BSTR* a_pbstrParams)
	{
		...
		bstr += sz2;
		if (m_fAngle != 0.0f || m_fAspectRatio != 1.0f)
		{
			swprintf(sz2, L",%g,%g", m_fAspectRatio, m_fAngle);
			bstr += sz2;
		}

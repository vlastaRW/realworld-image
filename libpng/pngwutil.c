...
#ifdef PNG_CUST_COMP_SUPPORTED
if (png_ptr->comp_deflate)
{
	png_bytep data = NULL;
	png_size_t len = 0;
	void (*free_fn)(png_byte_p) = NULL;
	int ret = png_ptr->comp_deflate(png_ptr->comp_data, png_ptr->comp_alloc, &data, &len, &free_fn);
	png_free(png_ptr, png_ptr->comp_data);
	png_ptr->comp_data = NULL;
	if (ret)
	    png_error(png_ptr, "compression error");
    png_write_IDAT(png_ptr, data, len);
	free_fn(data); // leaks if png_write_IDAT longjumps
}
else
{
#endif
...
#ifdef PNG_CUST_COMP_SUPPORTED
void png_set_cust_comp(png_structp png_ptr,
int (*comp_deflate)(png_bytep raw_data, png_size_t raw_size, png_bytepp comp_data, png_size_tp comp_size, void (**comp_free)(png_bytep))
)
{
	png_ptr->comp_deflate = comp_deflate;
	png_ptr->comp_data = NULL;
}
#endif
   ...
   png_size_t avail/*includes filter byte*/)
{
   ...
   /* Set up the zlib input buffer */
	...
	if (png_ptr->comp_deflate)
	{
		if (png_ptr->comp_data && png_ptr->comp_alloc >= png_ptr->comp_used+avail)
		{
			png_memcpy(png_ptr->comp_data+png_ptr->comp_used, filtered_row, avail);
			png_ptr->comp_used += avail;
		}
		else if (png_ptr->comp_data == NULL)
		{
			png_size_t alloc = (png_ptr->num_rows > 0 ? png_ptr->num_rows : 4)*avail;
			png_ptr->comp_data = png_malloc(png_ptr, alloc);
			if (png_ptr->comp_data == NULL)
			    png_error(png_ptr, "allocation error");
			png_ptr->comp_alloc = alloc;
			png_ptr->comp_used = avail;
			png_memcpy(png_ptr->comp_data, filtered_row, avail);
		}
		else
		{
			png_bytep temp = png_malloc(png_ptr, png_ptr->comp_alloc*2);
			if (temp == NULL)
			    png_error(png_ptr, "allocation error");
			png_memcpy(temp, png_ptr->comp_data, png_ptr->comp_used);
			png_memcpy(temp+png_ptr->comp_used, filtered_row, avail);
			png_free(png_ptr, png_ptr->comp_data);
			png_ptr->comp_data = temp;
			png_ptr->comp_alloc *= 2;
			png_ptr->comp_used += avail;
		}
	}
	else
